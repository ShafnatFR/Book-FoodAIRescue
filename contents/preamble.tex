...
\fancypagestyle{lscape}{
    ...
    \fancyhead[C]{
        \begin{tikzpicture}[remember picture,overlay]
            % Gray strip on physical left (= visual top in landscape view)
            \node[anchor=south west, fill=gray!20, minimum width=1.8cm, minimum height=\paperheight] at (current page.south west) {};
            % Logo centered in strip (0.9cm from left), at right margin of landscape content
            % yshift=-1.18in from north west = 1.18in from visual right = matches portrait right margin
            \node[anchor=center, rotate=90] at ([xshift=0.9cm, yshift=-1.18in]current page.north west) {
                \includegraphics[height=1.2cm]{image/Logo_Universitas_Telkom_Horizontal.png}
            };
        \end{tikzpicture}
    }
    \fancyfoot[C]{
        \begin{tikzpicture}[remember picture,overlay]
            % Page number at visual bottom-right (physical north east area), 1.18in from visual right
            \node[anchor=center, rotate=90] at ([xshift=-0.5in, yshift=-1.18in]current page.north east) {\thepage};
        \end{tikzpicture}
    }
    \renewcommand{\headrulewidth}{0pt}
    \renewcommand{\footrulewidth}{0pt}
