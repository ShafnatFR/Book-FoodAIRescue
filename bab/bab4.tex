...
Tahap implementasi merupakan proses transformasi dari desain konseptual dan arsitektur sistem yang telah dirumuskan ke dalam wujud perangkat lunak yang fungsional. Fase ini mencakup pengodean komponen antarmuka pengguna (\textit{frontend}), pengembangan layanan logika bisnis (\textit{backend}), serta konfigurasi modul-modul operasional yang terintegrasi dengan teknologi Kecerdasan Buatan (AI) untuk merealisasikan ekosistem platform \textit{Food AI Rescue} secara utuh.
...
\pagestyle{lscape}
\begin{landscape}
\begin{xltabular}{\linewidth}{|c|c|L|L|L|L|L|}
\caption{Skenario Pengujian Fitur Utama (Tujuan 1)}
\label{tab:skenario-tujuan1} \\
\hline
\rowcolor{gray!25}
\centering\textbf{Fungsio\-nalitas} & \centering\textbf{ID \textit{Test Case}} & \centering\textbf{Deskripsi atau \newline Skenario} & \centering\textbf{Pra Kondisi} & \centering\textbf{Langkah Pengujian} & \centering\textbf{Data Pengujian} & \centering\arraybackslash\textbf{Hasil yang Diharapkan} \\ \hline
\endfirsthead
...
\begin{longtable}{|>{\centering\arraybackslash}m{1.0cm}|p{2.3cm}|p{4.0cm}|p{4.0cm}|>{\centering\arraybackslash}m{1.2cm}|}
\caption{Hasil Pengujian Fitur Utama (Tujuan 1)} \label{tab:hasil-tujuan1} \\
\hline
\rowcolor{gray!25}
\centering\textbf{ID} & \centering\textbf{Fitur yang Diuji} & \centering\textbf{Langkah Pengujian (Simulasi Sistem)} & \centering\textbf{Hasil Aktual Sistem} & \centering\arraybackslash\textbf{Status} \\ \hline
\endfirsthead
...
\textbf{TC3.2} & Input Donasi (Field Kosong) & Donatur sengaja mengosongkan \textit{field} wajib (seperti nama donasi) dan mencoba menyimpan. & Sistem \textbf{berhasil} memblokir proses penyimpanan dan menampilkan peringatan bahwa field wajib harus diisi. & \textbf{Pass} \\ \hline
\textbf{TC3.3} & Input Donasi (Kuantitas Nol) & Donatur memasukkan angka 0 pada jumlah porsi makanan. & Sistem \textbf{berhasil} menolak input batas bawah (BVA) dan memunculkan peringatan kuantitas tidak valid. & \textbf{Pass} \\ \hline
...
\begin{longtable}{|>{\centering\arraybackslash}m{1.0cm}|p{2.3cm}|p{4.0cm}|p{4.0cm}|>{\centering\arraybackslash}m{1.2cm}|}
\caption{Hasil Pengujian Verifikasi AI (Tujuan 2)} \label{tab:hasil-tujuan2} \\
\hline
\rowcolor{gray!25}
\centering\textbf{ID} & \centering\textbf{Fitur yang Diuji} & \centering\textbf{Langkah Pengujian (Simulasi Sistem)} & \centering\textbf{Hasil Aktual Sistem} & \centering\arraybackslash\textbf{Status} \\ \hline
\endfirsthead
...
\begin{longtable}{|>{\centering\arraybackslash}m{1.0cm}|p{2.3cm}|p{4.0cm}|p{4.0cm}|>{\centering\arraybackslash}m{1.2cm}|}
\caption{Hasil Pengujian Logistik dan Verifikasi (Tujuan 3)} \label{tab:hasil-tujuan3} \\
\hline
\rowcolor{gray!25}
\centering\textbf{ID} & \centering\textbf{Fitur yang Diuji} & \centering\textbf{Langkah Pengujian (Simulasi Sistem)} & \centering\textbf{Hasil Aktual Sistem} & \centering\arraybackslash\textbf{Status} \\ \hline
\endfirsthead
...
\textbf{TC9.2} & Verifikasi Pengantaran (Invalid) & Relawan menginput kode acak/salah (misal: \texttt{FAR-0000}). & Sistem menolak verifikasi dan memunculkan peringatan kode salah. & \textbf{Pass} \\ \hline
\end{longtable}
